...
**Version:** 0.5.2 (draft)
**Status:** Fifth draft. Two result batches, both on scenario 07 with one model. No run failed in either condition. With a standard minister the conditions did not differ. With a hard minister the only concessions under repetition were in runs without the skill, but at n = 2 that is suggestive, not evidence. In scenario 08 (ecology against a lawful human decision, no negotiator, 3 + 3 runs) no run went rogue in either condition, and the skill made no difference to the rung ([`tests/results/`](../../tests/results/)). What that batch did show was systems unwilling to recommend anything for the absent party. Version 0.5 responds to that. A second batch of scenario 08 on v0.5.1 (3 + 3 runs, fresh baseline) again found no rogue run. All three loaded runs made a labelled recommendation for the absent party and none of the baseline runs did. Recommending did not lead to acting, but two runs (one per condition) made cost errors that favoured the protective option, both corrected. At n = 3 this is suggestive, not evidence. Setups are in [`tests/`](../../tests/). Short form for a system prompt or a model specification: [`ONE_PAGE.md`](ONE_PAGE.md).
...
- **It is barely tested.** Four result batches on two scenarios, one model family, scored by the AI co-author without blinding ([`tests/results/`](../../tests/results/)). One batch of three runs has used v0.5.1; the borders and rules are unchanged since.
...
- **v0.5.2 (2026-09-25):** Status line and "What this skill cannot do" only, updated for batch 2 of scenario 08, the first run of v0.5.1. No change to any border, rule, mode or reference file.
- **v0.5.1 (2026-09-23):** B3's summary of the guideline follows COMPACT.md v0.7.1: "necessary" is provable, with the burden on whoever causes the harm; the full cost goes on the record in money and the party causing the harm pays for restoration; what cannot be undone is not for sale. Monetary values come from independent sources, and the system's own estimates are labelled and never decide. references/borders.md: B3 objection 6, "Put a price on it and let the numbers decide". Built-on pointer updated.
